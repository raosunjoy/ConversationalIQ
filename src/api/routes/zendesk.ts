 ...
 */
async function validateZendeskAuth(req: Request, res: Response, next: NextFunction) {
  try {
...
zendeskRoutes.delete('/app/uninstall/:installation_id', (req: Request, res: Response) => {
 ...
 */
zendeskRoutes.get('/api/v1/sentiment/:conversationId', validateZendeskAuth, async (req: Request, res: Response) => {
  try {
    ...
    const messages = await dbService.findMessagesByConversation(conversationId!);
 ...
 */
zendeskRoutes.get('/api/v1/suggestions/:conversationId', validateZendeskAuth, async (req: Request, res: Response) => {
  try {
    const { conversationId } = req.params;
    ...
    const suggestions = await generateResponseSuggestions(recentMessages, installation.settings);
    ...
    res.json({
      ...
      context: {
        ...
        lastMessageSender: recentMessages[recentMessages.length - 1]?.senderType
      }
    });
  } catch (error) {
 ...
 */
zendeskRoutes.get('/api/v1/analytics/:conversationId', validateZendeskAuth, async (req: Request, res: Response) => {
  try {
    const { conversationId } = req.params;
    const installation = (req as any).zendeskInstallation;
 ...
 */
zendeskRoutes.post('/api/v1/events', validateZendeskAuth, async (req: Request, res: Response) => {
  try {
...
async function generateResponseSuggestions(
  ...
  if (lastMessage?.sentimentScore && lastMessage.sentimentScore < -0.5) {
    ...
    return suggestions.filter(s => s.category === 'empathy' || s.category === 'acknowledgment');
  }
 ...
 */
function calculateConversationAnalytics(messages: any[], conversation: any): any {
  const now = new Date();
  ...
  for (let i = 1; i < messages.length; i++) {
    ...
    if (current.sender === 'AGENT' && previous.sender === 'CUSTOMER') {
      const responseTime = (new Date(current.createdAt).getTime() - new Date(previous.createdAt).getTime()) / 1000;
  ...
  const negativeMessages = messages.filter(msg => msg.sentimentScore && msg.sentimentScore < -0.3);
  const escalationRisk = Math.min(negativeMessages.length / Math.max(messages.length, 1), 1);
  ...
  return {
    ...
    insights: [
      {
        type: 'trend',
        message: sentimentTrend > 0.1 ? 'Sentiment is improving' : 
                sentimentTrend < -0.1 ? 'Sentiment is declining' : 'Sentiment is stable'
      },
      {
        type: escalationRisk > 0.7 ? 'escalation' : 'positive',
        message: escalationRisk > 0.7 ? 'High escalation risk detected' : 
                escalationRisk > 0.4 ? 'Moderate escalation risk' : 'Low escalation risk'
      }
    ]
  };
}