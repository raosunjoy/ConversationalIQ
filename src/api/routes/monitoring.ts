    ...
    try {
      ...
      const systemHealth = {
        ...
        components: {
          ...
          encryption: {
            status:
              encryptionStats.totalKeysInCache > 0 ? 'healthy' : 'degraded',
            details: encryptionStats,
          },
        },
        summary: {
          healthyComponents:
            overallHealth.healthyChecks +
            (aiHealth.status === 'healthy' ? 1 : 0),
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const metrics = monitoringService.getMetricHistory(
        metricName || '',
        windowHours
      );
    ...
    try {
      ...
      const performance = {
        api: {
          responseTime: {
            avg:
              (dashboardData.metrics.operation_duration
                ?.slice(-10)
                ?.reduce((sum, m) => sum + m.value, 0) || 0) / 10,
            p95:
              dashboardData.metrics.operation_duration
                ?.slice(-10)
                ?.map(m => m.value)
          ...
          successRate:
            dashboardData.metrics.operation_success_rate?.slice(-1)[0]?.value ||
            0,
          throughput:
            (dashboardData.metrics.operation_count
              ?.slice(-10)
              ?.reduce((sum, m) => sum + m.value, 0) || 0) / 10,
        },
        ai: {
        ...
        system: {
          memoryUsage:
            dashboardData.metrics.memory_usage_percent?.slice(-1)[0]?.value ||
            0,
          cpuUsage:
            dashboardData.metrics.event_loop_lag?.slice(-1)[0]?.value || 0,
...
router.get('/sla', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const slaStatus = {
      ...
      summary: {
        meeting: Object.values(dashboardData.slas).filter(
          sla => sla.status === 'meeting'
        ).length,
        atRisk: Object.values(dashboardData.slas).filter(
          sla => sla.status === 'at_risk'
        ).length,
        violated: Object.values(dashboardData.slas).filter(
          sla => sla.status === 'violated'
        ).length,
      },
      timestamp: new Date(),
    };
    ...
    try {
      ...
      const alerts = {
        ...
        summary: {
          ...
          critical: dashboardData.recentAlerts.filter(
            a => a.rule.severity === 'critical'
          ).length,
          high: dashboardData.recentAlerts.filter(
            a => a.rule.severity === 'high'
          ).length,
          medium: dashboardData.recentAlerts.filter(
            a => a.rule.severity === 'medium'
          ).length,
          low: dashboardData.recentAlerts.filter(a => a.rule.severity === 'low')
            .length,
        },
        timestamp: new Date(),
      };
      ...
      const security = {
        compliance: {
          ...
          recentTests: complianceReport.controls
            ...
            .sort(
              (a, b) =>
                (b.lastTested?.getTime() || 0) - (a.lastTested?.getTime() || 0)
            )
            .slice(0, 5),
        },
        auditEvents: {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const resources = {
        memory: {
          current:
            dashboardData.metrics.memory_usage_mb?.slice(-1)[0]?.value || 0,
          percentage:
            dashboardData.metrics.memory_usage_percent?.slice(-1)[0]?.value ||
            0,
      ...
      if (format === 'prometheus') {
        ...
        for (const [name, metrics] of Object.entries(dashboardData.metrics)) {
          if (Array.isArray(metrics) && metrics.length > 0) {
            ...
            prometheusMetrics += ` ${latest?.value || 0}\n`;
          }
        }
