...
const conversationSlice = createSlice({
  ...
  reducers: {
    ...
    setConversationData: (state, action: PayloadAction<RealtimeConversationData>) => {
      state.conversationData = action.payload;
      state.lastUpdated = new Date();
      state.conversationError = null;
    },
    
    updateConversationData: (state, action: PayloadAction<Partial<RealtimeConversationData>>) => {
      if (state.conversationData) {
        state.conversationData = { ...state.conversationData, ...action.payload };
        state.lastUpdated = new Date();
      }
    },
    ...
    setMessages: (state, action: PayloadAction<MessageTimelineItem[]>) => {
      ...
      state.filteredMessages = filterMessages(action.payload, state.messageFilter);
    },
    ...
    addMessage: (state, action: PayloadAction<MessageTimelineItem>) => {
      ...
      if (existingIndex >= 0) {
        ...
        state.messages.sort((a, b) => a.timestamp.getTime() - b.timestamp.getTime());
      }
      state.filteredMessages = filterMessages(state.messages, state.messageFilter);
    },
    
    updateMessage: (state, action: PayloadAction<{ id: string; updates: Partial<MessageTimelineItem> }>) => {
    ...
    setMessageFilter: (state, action: PayloadAction<'all' | 'negative' | 'alerts'>) => {
    ...
    updateSentimentForMessage: (state, action: PayloadAction<{ messageId: string; sentiment: SentimentAnalysis }>) => {
    ...
    updateAnalytics: (state, action: PayloadAction<Partial<ConversationAnalytics>>) => {
      if (state.analytics) {
  ...
  switch (filter) {
    case 'negative':
      return messages.filter(msg => 
        msg.sentiment.label === 'NEGATIVE' && msg.sentiment.score < -0.3
      );
    case 'alerts':
...
// Selectors
export const selectCurrentConversationId = (state: { conversation: ConversationState }) => 
  state.conversation.currentConversationId;

export const selectConversationData = (state: { conversation: ConversationState }) => 
  state.conversation.conversationData;

export const selectMessages = (state: { conversation: ConversationState }) => 
  state.conversation.messages;

export const selectFilteredMessages = (state: { conversation: ConversationState }) => 
  state.conversation.filteredMessages;

export const selectMessageFilter = (state: { conversation: ConversationState }) => 
  state.conversation.messageFilter;

export const selectOverallSentiment = (state: { conversation: ConversationState }) => 
  state.conversation.overallSentiment;

export const selectSentimentHistory = (state: { conversation: ConversationState }) => 
...
export const selectIsLoadingConversation = (state: { conversation: ConversationState }) => 
  state.conversation.loadingConversation;

export const selectIsLoadingSentiment = (state: { conversation: ConversationState }) => 
  state.conversation.loadingSentiment;

export const selectIsLoadingAnalytics = (state: { conversation: ConversationState }) => 
