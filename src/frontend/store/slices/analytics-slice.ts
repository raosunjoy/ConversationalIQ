...
const analyticsSlice = createSlice({
  ...
  reducers: {
    ...
    updateResponseTime: (state, action: PayloadAction<number>) => {
      ...
      // Calculate average
      state.averageResponseTime = state.responseTimeTrend.reduce((sum, time) => sum + time, 0) / state.responseTimeTrend.length;
    },
...
export const { setAnalytics, updateResponseTime, setLoading, setError } = analyticsSlice.actions;
