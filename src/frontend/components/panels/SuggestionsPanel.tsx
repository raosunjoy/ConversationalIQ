...
import { insertTextIntoComment, showNotification } from '../../services/zendesk-auth';
...
const SuggestionsPanel: React.FC<SuggestionsPanelProps> = ({ 
  ...
  const appliedSuggestions = useAppSelector((state) => state.suggestions.appliedSuggestions);
  const feedbackGiven = useAppSelector((state) => state.suggestions.feedbackGiven);

  // Query existing suggestions
  const { data: suggestionsData, loading: queryLoading, refetch } = useQuery(
    GET_RESPONSE_SUGGESTIONS,
    {
  ...
  const handleProvideFeedback = async (suggestionId: string, rating: number, helpful: boolean) => {
      ...
      <div className="suggestions-container">
        {suggestions.length === 0 ? (
          <div className="suggestions-empty">
            ...
            <p>AI suggestions will appear here when conversation analysis is complete.</p>
            <Button 
              isPrimary
              onClick={handleRefreshSuggestions}
              disabled={refreshing || !conversationId}
            ...
            {suggestions
              ...
              .map((suggestion) => (
                <SuggestionCard
                  ...
                  onFeedback={(rating, helpful) => handleProvideFeedback(suggestion.id, rating, helpful)}
